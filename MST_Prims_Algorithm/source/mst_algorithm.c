...
pathtype_t find_min_edge_in_active_nodes(treetype_t *mst)
	{
		...
		for (int src_node = 0; src_node < NUMBER_OF_NODES; src_node++)
			{
				if (mst->nodes[src_node] == NODE_IS_ACTIVE)
					{
						for (int dest_node = 0; dest_node < NUMBER_OF_NODES;
								dest_node++)
							{
								if ((edges[dest_node][src_node] < min)
										&& (mst->nodes[dest_node]
												== NODE_IS_NOT_ACTIVE))
									{
...
void add_path_to_tree(treetype_t *mst, pathtype_t *path_result)
	{
		...
		mst->tree[path_result->dest_index][path_result->src_index] = edges[path_result->dest_index][path_result->src_index];
